...
const RegionGallery = () => {
  ...
  const endpoint = `${process.env.REACT_APP_BACKEND_URL}/publications/region/${regionId}`;
      ...
      {isLoading ? (
        ...
        <div>
          {publications.length > 0 ? (
            <>
            <h1 className="innova-heading text-center text-3xl font-bold text-blue-800 my-10">Resultados para la región:  {publications[0].location.regionId}</h1>
            ...
            <h3 className="innova-heading text-center text-3xl font-bold text-blue-800 my-10">
              No se encontraron resultados para esta region
            </h3>
          )}
        </div>
      )}
